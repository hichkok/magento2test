...
/*------------------------------------------------------------------------------+
| MagneticOne                                                                   |
| Copyright (c) 2016 MagneticOne.com <contact@magneticone.com>                  |
| All rights reserved                                                           |
+-------------------------------------------------------------------------------+
...
class M1_Config_Adapter
{
  ...
  public function create()
  {
    ...
    if (M1_Config_Adapter::detectCartType() == 'PrestaShop') {
      return $obj;
    }
...
class M1_Mysql
{
  ...
  public function query($sql, $fetchType)
  {
    ...
    if (isset($_GET['disable_checks'])) {
      mysql_query('SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0', $dataBaseHandle);
      mysql_query("SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO'", $dataBaseHandle);
    }
    ...
    if (isset($_GET['disable_checks'])) {
      mysql_query("SET SQL_MODE=IFNULL(@OLD_SQL_MODE,'')", $dataBaseHandle);
      mysql_query("SET FOREIGN_KEY_CHECKS=IFNULL(@OLD_FOREIGN_KEY_CHECKS,0)", $dataBaseHandle);
    }
  ...
  public function __construct($config)
  {
    ...
    switch ($this->getAction()) {
      case 'savefile':
        break;
      case 'update':
        $updateAction = new M1_Bridge_Action_Update();
        $updateAction->perform($this);
        break;
      case 'move':
        $moveAction = new M1_Bridge_Action_Move();
        $moveAction->perform($this);
        break;
      case 'getconfig':
        $getConfigAction = new M1_Bridge_Action_Getconfig();

        if (!$this->config->cartVars['dbCharSet']) {
          $this->config->cartVars['dbCharSet'] = $this->config->getCharsetFromDb();
        }

        $getConfigAction->perform($this);
        die();
      default:
...
class M1_Config_Adapter_PrestaShop extends M1_Config_Adapter
{
  ...
  public function __construct()
  {
    ...
    $link = $this->connect();
    if ($link) {
      $row = $link->localQuery("SELECT value FROM " . _DB_PREFIX_ . "configuration WHERE name = 'PS_WEBSERVICE'");
      if (isset($row[0]) && $row[0]['value'] == 1) {
        $this->cartType = 'PrestaShopCloud';
      }
    } else {
      $this->cartType = 'PrestaShop';
    }

    if (file_exists(M1_STORE_BASE_DIR . '/classes/Dispatcher.php')) {
...
class M1_Bridge_Action_Clearcache
{
  ...
  public function prestashopClearCache()
  {
    ...
    $this->removeGarbage($dirs, 'index.php');
  }
  ...
  /**
   * clear Xtcommerce4 cache
   *
   * @return void
   */
  public function xtcommerce4ClearCache()
  {
    $dirs = array(
...
$actionsIgnoreConfig = array(
  'move',
);

if (!isset($_GET['action']) || !in_array($_GET['action'], $actionsIgnoreConfig)) {
